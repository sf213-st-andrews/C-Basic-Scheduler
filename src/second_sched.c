// sf213
// second step, with data structures
...
#define CONF                    "chars.conf"    // filename of conf file
...
void work_Process(Process* p, int quantum) {
    kill(p->pid, SIGCONT);
    usleep(quantum);
    kill(p->pid, SIGSTOP);   
}
